...
class Order extends AbstractPdf
{
    ...
    protected function _drawHeader(\Zend_Pdf_Page $page)
    {
        ...
        $lines[0][] = ['text' => __('Price'), 'feed' => 480, 'align' => 'right'];
    ...
    protected function _drawItem($item, $page, $order)
    {
        ...
        $lines[0][] = ['text' => $sku, 'feed' => 70, 'align' => 'left'];

        $lines[0][] = ['text' => $item->getPopName(), 'feed' => 200, 'align' => 'left'];

        $lines[0][] = ['text' => $order->getCurrency()->format($item->getPopPrice(), [], false), 'feed' => 480, 'align' => 'right'];
    ...
    {
        return $this->_config->create()->getSetting('pdf/billing_address', $order->getpo_store_id());
    }

    public function getShippingAddress($order)
    {
        return $this->_config->create()->getSetting('pdf/shipping_address', $order->getpo_store_id());
    }
    ...
    protected function drawPoInformation($page, $order)
    {
        ...
        $additionnalTxt[] = __('Estimate time of reception : %1', $order->getpo_eta());

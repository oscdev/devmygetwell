...
class Quote
{
    ...
    public function addProduct($productId, $qty, $customPrice = null, $shipLater = null)
    {
        ...
        $result->setPosShipLater($shipLater)->save();
    ...
    public function addOrUpdateProduct($productId, $qty, $customPrice, $shipLater = false)
    {
        //$product = $this->_product->create()->load($productId);
        ...
        if (!$quoteItem)
        {
            ...
            if ($qty > 0) {
                ...
                if ($customPrice)
                    $request->setCustomPrice($customPrice);
                $this->addProduct($productId, $qty, $customPrice, $shipLater);
        ...
        else
        {
            ...
            if ($qty > 0)
            {
                $hasChanges = (
                                ...
                                || ($customPrice != $quoteItem->getcustom_price())
                                || ($shipLater != $quoteItem->getPosShipLater())
                                );
                if ($hasChanges)
                {
                    $request =  new \Magento\Framework\DataObject();
                    ...
                    if ($customPrice)
                        $request->setCustomPrice($customPrice);
                    $quoteItem = $this->_quote->updateItem($quoteItem->getId(), $request);
                    $quoteItem->setPosShipLater($shipLater)->save();
                    $this->_logger->log('Update product #'.$productId.' for quote item #'.$quoteItem->getId().' (qty: '.$qty.', shipLater: '.$shipLater.')');
                }
                else
    ...
    public function getResult()
    {
        ...
        foreach($this->_quote->getAllItems() as $item)
        {
            ...
            $quoteItem['ship_later'] = $item->getPosShipLater();
    ...
    public function getProductIdsToShip()
    {
        $productIds = [];

        foreach($this->_quote->getAllItems() as $item)
        {
            if (!$item->getPosShipLater())

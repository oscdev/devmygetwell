...
class SourceLocation extends \Magento\Backend\Block\Widget\Grid\Column\Renderer\AbstractRenderer {
    ...
    public function render(\Magento\Framework\DataObject $row){
        ...
        $html = '';
        $html .= __('Qty').' : '.$row->getSourceWarehouse()->getwi_physical_quantity().'<br/>';
        if ($row->getSourceWarehouse()->getwi_shelf_location())
            $html .= __('Location').' : '.$row->getSourceWarehouse()->getwi_shelf_location();
        return $html;
